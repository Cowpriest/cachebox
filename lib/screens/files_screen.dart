...
class BreadcrumbChips extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    // Visuals
    final outline =
        Theme.of(context).colorScheme.outlineVariant.withOpacity(.5);
...
class _FilesScreenState extends State<FilesScreen> {
          ...
          for (final folder in listing.folders) {
            items.add(
              ListTile(
                ...
                subtitle: folder.childrenCount != null
    ...
    try {
      ...
      if (['.png', '.jpg', '.jpeg', '.gif', '.webp'].contains(ext)) {
        ...
        Navigator.push(
            ...
            MaterialPageRoute(
                builder: (_) =>
                    VideoPlayerScreen(videoUrl: url, fileName: file.fileName)));
      } else if (ext == '.pdf') {
        ...
        Navigator.push(
            ...
            MaterialPageRoute(
                builder: (_) => PdfViewerScreen(
                    filePath: tmpPath, fileName: file.fileName)));
      } else {
        final response = await http.get(Uri.parse(url));
        if (response.statusCode != 200) {
        ...
        Navigator.push(
            ...
            MaterialPageRoute(
                builder: (_) => TextViewerScreen(
                    textContent: text, fileName: file.fileName)));
      }
    } catch (e) {
      setState(() => _busy = false);
      if (!mounted) return;
  ...
  Future<void> _pickAndUpload() async {
    ...
    try {
      ...
      final targetName = _currentPath.isNotEmpty
          ? widget.normalizePosix(widget.joinPosix(_currentPath, picked.name))
          : picked.name;
